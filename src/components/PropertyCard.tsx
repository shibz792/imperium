...
import { completenessScore, isStale, primarySize, priceUnit, relevantAskingPrice } from "@/lib/property";
...
export function PropertyCard({ property }: { property: Property & { assignedAgent?: { name: string } | null } }) {
  ...
  return (
    <Link href={`/properties/${property.id}`} className="ir-card ir-card-hover group flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 flex-col p-4">
        <h3 className="mb-1 line-clamp-2 text-sm font-semibold leading-snug text-ir-navy group-hover:text-ir-gold-dark">{property.title}</h3>
